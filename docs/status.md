...
- [ ] Sprint B leftovers: rebase `CommandError` (`modules/deployment/docker.ts`) on `DomainError`; delete the ~21 redundant `error instanceof Error ? error.message` router re-wraps; preview deployments carry only `commitSha` (no message/author); compose commit links skip self-hosted GitLab/Gitea (`compose.one` loads no provider relation); streaming dumps to S3 (architecture #18) still open — ~37 MB ceiling documented.
...
- [ ] `schedule.create/remove` write no audit rows. (`settings/incidents/page.tsx` is now a `redirect()` stub into Monitoring, so the missing `<title>` is moot.)
