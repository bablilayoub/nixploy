...
- [ ] Sprint B leftovers: rebase `CommandError` (`modules/deployment/docker.ts`) on `DomainError`; delete the ~21 redundant `error instanceof Error ? error.message` router re-wraps; sweep the remaining `toast.error(error.message)` sites (about 100 in 35 files, listed in the 2026-09-11 error-boundary commit body context) to `toastError`; `overview-section.tsx` still polls `deployment.recent` every 10 s; preview deployments carry only `commitSha` (no message/author); compose commit links skip self-hosted GitLab/Gitea (`compose.one` loads no provider relation); streaming dumps to S3 (architecture #18) still open — ~37 MB ceiling documented.
...
0. **Work the improvement audit** — [`audits/2026-09/README.md`](./audits/2026-09/README.md). Sprint A (§1, all 15 items) landed 2026-09-11 (commits `8b5c899`…`3844db6`); Sprint B landed 2026-09-11 (`fe7b566`…`89ca766`: error boundary, deploy provenance + pre-flight, backup run history + local destination + verify, half-built features, shared org env). Sprint C (network isolation + container hardening, durable queue + metrics store, Traefik middleware layer, unified service pages) in progress; then §3.4+ (worker split, SSH pool, push events), §4 steps 2-5, remaining §2 (scoped keys/SSO, CLI/MCP parity, wildcard) and §6.
