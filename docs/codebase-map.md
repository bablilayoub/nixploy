...
| `templates` (remote kinds) | `modules/templates/sources.ts` (`http-json` / `git` / `blueprints` sync, cache under `<config>/templates/sources`), `blueprints.ts` (the Dokploy catalog layout → `Template`, pure), `toml.ts` (the TOML subset those files use), `placeholders.ts` (deploy-time env placeholders: named generators, `{{domain}}`, `{{env:KEY}}`, JWT) |
