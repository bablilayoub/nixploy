...
*The offline-dump path landed 2026-09-20:* `POST /api/import/dump` stores a `pg_dump` (plain, gzipped or custom) under the organisation for 24 hours; `DumpSourceReader` restores it into a throwaway `postgres:17-alpine` on `--network none`, reads the rows with `row_to_json` — the source's drizzle rows carry the same camelCase names its API returns, so the live path's lenient schemas parse them unchanged — discovers the child tables by name, and removes the container. One `SourceReader` interface sits behind `inspect` / `plan` / `apply`, so the translation, notes and plan are identical for both paths. Verified against a synthetic dump shaped like the source's tables in a real container, not a real dump; the keyring for encrypted env values is still not read (values arrive as keys with a note).
...
*Also landed 2026-09-20, outside the original list:* **DNS records created for you** — the DNS-01 provider link writes the A record for every attached domain (Cloudflare, DigitalOcean, Hetzner Cloud, Vultr, Gandi; off by default, best-effort by contract, `domain.ensureDnsRecord` to retry) — and **template domain hints**, so a template declares the hostnames it reads from its env values (OpenHole's endpoint host and `*.<tunnel domain>`) and they are attached, certificated and resolved on deploy.
...
### 15. Ephemeral environments ✅ *(L — landed 2026-09-20)*
...
*What shipped (migration 0043):* `preview_deployment.kind` (`pull_request` | `branch`); a branch preview is created from any ref and keyed by a short hash of it (`b<6 hex>` in the key column every consumer already names variants after), so the service name, hosts, Traefik keys and the reserved app-name pattern all kept one shape — the only change to them is the key alphabet. PR-only machinery (the comment, the fork gate, webhook teardown) checks the kind. `previewDeployment.redeploy` is exposed; `create` returns the `deploymentId`, so `nixploy preview create --ref … --wait` gets the existing machine-readable outcome (status, failing step, log tail, URLs, health) — the `{url, health, logTail}` the sketch asked for is that outcome. MCP `create_preview` / `get_preview` / `delete_preview`. **Commit statuses** landed the same day (`modules/preview/status.ts`): `nixploy/preview` on the commit, pending → success (linking the preview host) / failure, for all four providers and for branch previews too, best effort. **The per-preview database** landed too (migration 0044): `previewDatabaseKind` / `previewDatabaseId` / `previewSeedCommand` on the parent, a `database_logical` row per preview created on first deploy (approval included) and dropped with it, `DATABASE_URL` injected over everything, the seed once per preview through the pre-deploy hook machinery (application) or an exec into the preview project after `up` (compose). **Image previews** closed the section the same day: `kind = image`, keyed by a hash of the image ref, the deploy target becomes a docker-source job for that deployment only (`buildPreviewDeployTarget`), so a git-source parent previews a CI-built image with no build; applications only, since a compose stack has no single image.

### 16. Copilot v2 *(L — the route diagnostician and propose-and-approve remediation landed 2026-09-20)*
...
*What shipped:* the no-LLM half — `modules/traefik/diagnose.ts`, `domain.diagnose`, the stethoscope on every domain row, `nixploy domain diagnose`, MCP `get_domain_diagnosis` (42 tools). Eight probes in request order — DNS, route file, a second file claiming the host, upstream task, shared-network attachment, container port (a throwaway busybox `nc` on the overlay), Traefik's own answer with the Host header set (404 → no router, 502/503 → no healthy server), certificate — each a finding with its fix, aggregated into healthy / degraded / broken. Traefik's API stays disabled, so the verdict comes from the file and an end-to-end request rather than from asking the proxy. The evidence half moved the same day: the failure explainer and the service chat now carry the runtime log tail (what the app printed before the deploy ended / right now) next to the event timeline, redacted like the build log. **Propose-and-approve landed 2026-09-20** (`modules/remediation/`): the restart-loop rule (3 task failures or OOM kills in 10 minutes, one proposal per service per hour, never while a deployment is in flight) files an incident of kind `remediation` carrying the action — *roll back to the previous pinned image* for an application, *restore the earlier snapshot* for a compose stack, nothing but an explanation for an OOM loop or a service with no earlier deploy — and a human applies or dismisses it from the activity tray, Monitoring → Incidents, `nixploy incident apply|dismiss` or the API. Applying runs the exact code the manual rollback uses. `NIXPLOY_REMEDIATION=0` is the kill switch. The second rule followed the same day: a deploy that ends in error at the **rollout**, **converge** or **post-deploy** step proposes going back to the image the service ran before it (a build failure never does — nothing changed on the cluster), and it wins over the restart loop when both fire. Metrics and route state joined the context on 2026-09-20: the service chat now carries six hours of resource use summarized to three facts (now, peak, the drift between the window's halves) and the deterministic route verdict for up to three of its domains, failing probes only. §16 is done.
...
- **Footprint budget.** ✅ measured (2026-09-19): the e2e job reads the panel container's resident memory after the full golden path and fails over `NIXPLOY_PANEL_RSS_BUDGET_MIB` (768), printing the figure in the job summary. Today: 427 MiB panel + 51 Postgres + 26 Traefik = ~505 MiB on the live VPS. `NIXPLOY_LITE` landed 2026-09-20 (`lib/profile.ts` + `install.sh --lite`): a profile that moves the defaults of runtime-log harvesting, metric sampling and retention, the uptime pass and the SSH channel cap for a 1–2 GB box, with every knob still settable on its own. Still open: a measured figure for that profile, and publishing the numbers on the site. Figures from the e2e job on 2026-09-19/20 (`panel memory:` in the job log): 419, 430, 434 and one 580 MiB — the outlier came from the run that added the log harvester, but the next runs with the same code measured 434, so it reads as GC timing rather than a step; the ceiling stays at 768 until the outlier repeats or stays away over a week.
...
- **Known-regressions notes** in every release, and an in-panel preflight before an update. ✅ preflight (2026-09-20): the Update dialog runs disk / registry / deployments / backup age / readiness / downgrade / release-notes checks; blocks disable the button and the low-disk block is enforced by the updater itself (docs/install.md → Upgrade safety). Known-regressions notes remain a release-process habit, not code.
