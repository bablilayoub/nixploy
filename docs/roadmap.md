...
- **Footprint budget.** ✅ measured (2026-09-19): the e2e job reads the panel container's resident memory after the full golden path and fails over `NIXPLOY_PANEL_RSS_BUDGET_MIB` (768), printing the figure in the job summary. Today: 427 MiB panel + 51 Postgres + 26 Traefik = ~505 MiB on the live VPS. Still open: a `NIXPLOY_LITE` profile and publishing the number on the site. Figures from the e2e job on 2026-09-19/20 (`panel memory:` in the job log): 419, 430, 434 and one 580 MiB — the outlier came from the run that added the log harvester, but the next runs with the same code measured 434, so it reads as GC timing rather than a step; the ceiling stays at 768 until the outlier repeats or stays away over a week.
