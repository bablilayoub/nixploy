...
pnpm exec biome check --error-on-warnings packages/server apps/web apps/cli apps/landing   # what CI runs (0 errors, 0 warnings)
pnpm knip                                      # dead files/exports/deps (pnpm dlx knip@5.88.1, also a CI step)
...
- **Tooling (2026-09-11)**: `biome.json` is strict JSON — a `//` comment makes Biome silently fall back to its default config (it then lints `.next/` and reports hundreds of thousands of errors, no parse error printed). `biome check` exits 0 on warnings unless `--error-on-warnings` is passed (CI and `pnpm lint` pass it). `apps/web`'s tsconfig also governs how `@nixploy/server`'s raw TypeScript is typechecked (`transpilePackages`), so panel-side strictness flags such as `noUncheckedIndexedAccess` apply to the server sources too. App-name generation lives in `modules/services/app-name.ts` (one namespace across all seven kinds). `CommandError` extends `DomainError` (timeouts map to TIMEOUT) — do not re-wrap command failures in routers. Drizzle's `primaryKey` import trips Biome's `noDeprecatedImports` even in the object form; the rule stays off.
