...
- `deploymentStatus` is `queued → running → done | error | cancelled`. Rows are inserted as `queued`; the worker flips them to `running`. Boot recovery re-enqueues leftover `queued` rows and fails leftover `running` ones. One `queued` + one `running` job per app at most (newer pushes supersede the queued one); same-app jobs never run concurrently. The in-memory queue state lives on `globalThis` because Next's `transpilePackages` evaluates `packages/server` modules twice (route chunks vs `server.ts`) — any new process-wide singleton in `packages/server` must do the same (see `deployment/events.ts`, `queue.ts`).
