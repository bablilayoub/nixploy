...
const COOLIFY: Comparison = {
	...
	facts: [
		...
		},
		{
			...
			ours: "36 MCP tools, each carrying hand-declared readOnlyHint / destructiveHint / idempotentHint with a test that fails the build on a missing entry, plus task tools that return a deploy's real outcome in one call instead of a polling loop.",
